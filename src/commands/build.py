...
from src.constants import URI, Constants, Paths, SetData
from src.icons import get_missing_icons, get_missing_rarities
from src.schema import Manifest, ManifestMeta, ManifestSet, ManifestWatermark
...
from src.watermarks import get_local_watermarks, get_missing_watermarks
...
def build_docs() -> None:
    ...
    with open(Paths.DOCS_MISSING, "w", encoding="utf-8") as file:
        ...
        for sym in icons:
            file.write(
                f"| {sym.icon} | [SVG]({sym.svg_url}), [Cards]({sym.set_url}) |\n"
            )
        ...
        for sym in rarities:
            file.write(
                f"| {sym.icon.upper()} | {sym.missing_str} | "
...
def build_manifest(date: str) -> None:
    ...
    manifest: Manifest = Manifest(
        ...
        watermark=ManifestWatermark(routes={}, symbols=sorted(get_local_watermarks())),
    )
    ...
    # Create packages
    create_zip(src=Paths.SVG, dst=Paths.PACKAGE_ALL, files=[Paths.MANIFEST])
    create_zip(
        src=Paths.SVG_OPTIMIZED, dst=Paths.PACKAGE_OPTIMIZED, files=[Paths.MANIFEST]
    )
...
def build_optimized(npm_command: str | None = None) -> None:
